...
which other authorities you want to do the test using the `--peers`. And you 
can execute each of the steps individually using `create`, `encrypt` or `tally`
options instead of `full`. Find more information with `eotest --help`.
:::
...
For troubleshooting please read the [Deployment Troubleshooting](deployment_troubleshooting.md).
